...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Subsamples a large population based on the characteristics of a small population.')
    ...
    assert len(df_population.columns) == len(df_columns.columns) == len(df_sample_condition.columns), "All data files must have the same number of columns"
    #make sure that population file has at least one free choice in it
    assert len(df_population)>len(df_sample_condition), "The population file must have more items in it than the sample condition file"
    ...
    print "Building difference tables matrix of size (" + str(len(df_population)) + "x" + str(len(df_population)) +"):",
    x_i=0  
